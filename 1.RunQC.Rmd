...
if (params$local) {
  ...
  directory = list(raw = sprintf("%s/cellranger/%s/outs", params$baseDir, run2process),
                   rds = sprintf("%s/seurat_obj/basicObj_%s", params$baseDir, Sys.Date()))
}
...
if (!dir.exists(directory$rds)) {
  dir.create(directory$rds)
  if (!dir.exists(directory$rds)) {
...
filename = sprintf("%s_1.SeuratObj_%s.rds", file2save, Sys.Date())

print(sprintf("Saving %s to file %s at %s", run2process, filename, directory$rda))

saveRDS(seuratObj, sprintf("%s/%s", directory$rda, filename))
```
